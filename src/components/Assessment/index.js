  ...
  checkError = () => {
    ...
    for (i = 0; i < catg_list.length; i++) {
      ...
      for (j = 0; j < catg_err.length; j++) {
        if (catg_err[j].error || catg_err[j].difficulty_error) return i
      }
    }
    return i
  }

  submitResponse = (isFinalSubmit) => {
    let response = {}
    ...
    for (let catg in current_values) {
      ...
      catg_values.forEach((ele) => {
        ...
        if (
          (ele.implementation == 'not' || ele.implementation == 'partially') &&
          ele.difficulty == ''
        ) {
        ...
        for (let phase = 0; phase < phases.length; phase++) {
          for (let k = 0; k < current_values[phases[phase]].length; k++) {
            ...
            switch (this.props.assessmentState) {
              case 'identify':
                prefix = 'i'
                difficulty_prefix = 'id'
                break
              case 'protect':
                prefix = 'p'
                difficulty_prefix = 'pd'
                break
              case 'detect':
                prefix = 'd'
                difficulty_prefix = 'dd'
                break
              case 'response':
                prefix = 'r'
                difficulty_prefix = 'rd'
                break
              case 'recovery':
                prefix = 're'
                difficulty_prefix = 'red'
                break
              default:
                prefix = ''
                difficulty_prefix = ''
            }
            response[`${prefix}${i_index}`] = implementation_score
            response[`${difficulty_prefix}${d_index}`] = difficulty_score
            response['email'] = this.props.user && this.props.user.email
          }
        }
        console.log(response)
        axios
          .post(`/api/${this.props.assessmentState}`, response)
          .then((res) => {
            console.log(res.data)
            this.props.updateResult({ [this.props.assessmentState]: res.data })
            if (isFinalSubmit) this.props.changeDashboardState('result')
            ...
            <div className="phase-container">
              ...
              <div className="parts-container">
                ...
                <div className="buttons-container">
                  ...
                  <div
                    onClick={() => {
                      this.submitResponse(!this.props.showNext)
                    }}
                    className="next-button changeState"
                  >
                    {this.props.showNext ? 'Next' : 'Submit'}
                  </div>
                </div>
              </div>
            </div>
